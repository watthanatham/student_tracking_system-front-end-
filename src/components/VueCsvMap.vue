<template>
    <slot sample="VueCsvImportData.firstSampleRow" :map="VueCsvImportData.map" :fields="VueCsvImportData.fields">
          ...
          if (newVal) {
            VueCsvImportData.fields.forEach((field) => {
              newVal[0].forEach((columnName, index) => {
                const fieldName = props.autoMatchIgnoreCase
                  ? field.label.toLowerCase().trim()
                  : field.label.trim()
                const colName = props.autoMatchIgnoreCase
